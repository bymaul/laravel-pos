...
Route::group(['middleware' => ['auth', 'role:admin']], function () {
    ...
    Route::post('/product/delete-selected', [ProductController::class, 'deleteSelected'])
        ->name('product.delete-selected');
    Route::resource('/product', ProductController::class);

    Route::get('/report', [ReportController::class, 'index'])
        ->name('report.index');
    Route::get('/report/data/{startDate}/{endDate}', [ReportController::class, 'data'])
        ->name('report.data');

    Route::get('/report/export/{startDate}/{endDate}', [ExportController::class, 'export'])
        ->name('report.export');
...
Route::middleware('auth')->group(function () {
    Route::get('/transaction/new', [SaleController::class, 'create'])
        ->name('transaction.new');
    Route::post('/transaction/save', [SaleController::class, 'store'])
        ->name('transaction.save');

    Route::get('/transaction/{id}/data', [SaleDetailController::class, 'data'])
        ->name('transaction.data');
    Route::get('/transaction/loadform/{total}/{received}', [SaleDetailController::class, 'loadForm'])
        ->name('transaction.load-form');
